...
class CompressionModel(nn.Module):
    ...
    def update(self, force_zero_thres=None, frozen_state=None):
        self.entropy_coder = EntropyCoder()
        if frozen_state is not None:
            self.load_frozen_entropy_state(frozen_state, force_zero_thres=force_zero_thres)
            return
    ...
    def export_frozen_entropy_state(self):
        if self.entropy_coder is None:
            raise RuntimeError("Entropy coder is not initialized. Call update() before exporting.")

        return {
            "format_version": 1,
            "model_type": type(self).__name__,
            "z_channel": self.z_channel,
            "gaussian_encoder": self.gaussian_encoder.export_state(),
            "bit_estimator_z": self.bit_estimator_z.export_state(),
        }

    def save_frozen_entropy_state(self, path):
        torch.save(self.export_frozen_entropy_state(), path)

    def load_frozen_entropy_state(self, frozen_state, force_zero_thres=None):
        if isinstance(frozen_state, (str, os.PathLike)):
            frozen_state = torch.load(frozen_state, map_location=torch.device('cpu'),
                                      weights_only=False)

        if frozen_state["z_channel"] != self.z_channel:
            raise RuntimeError(
                f"Frozen entropy state mismatch: expected z_channel={self.z_channel}, "
                f"got {frozen_state['z_channel']}"
            )

        self.gaussian_encoder.load_state(
            frozen_state["gaussian_encoder"],
            self.entropy_coder,
            force_zero_thres=force_zero_thres,
        )
        self.bit_estimator_z.load_state(frozen_state["bit_estimator_z"], self.entropy_coder)
