...
class AEHelper():
    ...
    def export_cdf_info(self):
        if self._quantized_cdf is None or self._cdf_length is None or self._offset is None:
            raise RuntimeError("CDF info is not initialized. Call update() before exporting.")

        return {
            "quantized_cdf": np.array(self._quantized_cdf, copy=True),
            "cdf_length": np.array(self._cdf_length, copy=True),
            "offset": np.array(self._offset, copy=True),
        }

    def restore_cdf_info(self, cdf_info, entropy_coder):
        quantized_cdf = torch.as_tensor(cdf_info["quantized_cdf"], dtype=torch.int32)
        cdf_length = torch.as_tensor(cdf_info["cdf_length"], dtype=torch.int32)
        offset = torch.as_tensor(cdf_info["offset"], dtype=torch.int32)
...
class BitEstimator(AEHelper, nn.Module):
    ...
    def export_state(self):
        return {
            ...
            "cdf_info": self.export_cdf_info(),
        }

    def load_state(self, state, entropy_coder):
        if state["qp_num"] != self.qp_num or state["channel"] != self.channel:
            raise RuntimeError(
                f"Frozen BitEstimator state mismatch: expected qp_num={self.qp_num}, "
                f"channel={self.channel}, got qp_num={state['qp_num']}, "
                f"channel={state['channel']}"
            )

        self.restore_cdf_info(state["cdf_info"], entropy_coder)
...
class GaussianEncoder(AEHelper):
    ...
    def export_state(self):
        return {
            ...
            "log_scale_step": self.log_scale_step,
            "log_step_recip": self.log_step_recip,
            "force_zero_thres": self.force_zero_thres,
            "cdf_info": self.export_cdf_info(),
        }

    def load_state(self, state, entropy_coder, force_zero_thres=None):
        if state["scale_level"] != self.scale_level:
            raise RuntimeError(
                f"Frozen GaussianEncoder state mismatch: expected scale_level={self.scale_level}, "
                f"got {state['scale_level']}"
            )
        if abs(state["scale_min"] - self.scale_min) > 1e-8 or \
                abs(state["scale_max"] - self.scale_max) > 1e-8:
            raise RuntimeError(
                f"Frozen GaussianEncoder state mismatch: expected scale range "
                f"[{self.scale_min}, {self.scale_max}], got "
                f"[{state['scale_min']}, {state['scale_max']}]"
            )

        self.force_zero_thres = state["force_zero_thres"] \
            if force_zero_thres is None else force_zero_thres
        self.restore_cdf_info(state["cdf_info"], entropy_coder)
